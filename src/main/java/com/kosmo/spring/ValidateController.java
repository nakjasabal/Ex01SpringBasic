...
public class ValidateController {
	...
	폼값을 전송했을때의 요청처리
	메서드의 첫번째 파라미터
		: 폼값을 커맨드 객체를 통해 한번에 받은 후 어노테이션으로 모델객에에
		저장시 mInfo로 이름을 변경한다. 
	두번째 파라미터
		: BindingResult 객체는 validator를 통해 폼값을 검증한 후 해당 결과를
		전송받는 객체로, 폼값에 문제가 있다면 해당 객체를 통해 확인할 수 있다.
	세번째 파라미터
	...
	public String registProc(
		...
		//폼값검증 완료시 View로 반환할 페이지의 경로
		String viewPage = "03Validate/memberDone";
		
		/*
		유효성 검증을 위해 정의한 클래스의 객체를 생성한 후 전송된 폼값을 저장한
		커맨드객체를 통해 폼값의 유효성 체크를 실시한다. 매개변수로 유효성 체크를
		할 커맨드 객체와 바인딩 결과(검증결과)를 저장할 객체를 인수로 전달한다. 
		 */
		...
		if(result.hasErrors()){
			...
			//가입 페이지를 다시 브라우저에 포워드한다. 
			viewPage = "03Validate/memberRegist";
		}
		
		
		//검증에 문제가 없다면 완료페이지로 포워드한다. 
		return viewPage;
	}
}
