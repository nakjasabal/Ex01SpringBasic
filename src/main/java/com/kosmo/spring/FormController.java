...
com.kosmo.spring을 스캔하여 컨트롤러 클래스를 찾아낸다. 그리고 해당 요청명에 
매핑된 메서드를 찾아 요청을 전달하고 로직을 수행한다.

@Controller
	: 해당 클래스를 컨트롤러로 사용하고 싶을때 클래스명 앞에 선언한다. 
	패키지를 스캔할때 해당 어노테이션이 있는 클래스를 찾아서 요청을 전달하게된다. 

@RequestMapping
	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지로 컨택스트루트를 제외한 나머지
	경로명으로 이루어진다. 요청명의 매핑정보로 메서드를 호출하므로 메서드명은 큰 
	의미가 없다. 개발자가 구분하기 좋은 정도의 이름으로 설정하면된다. 
 */
@Controller
public class FormController {
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		/*
		View의 페이지 경로를 반환한다. 아래처럼 String으로 반환하면 ViewResolver가
		전체경로를 조립하여 웹브라우저에 출력한다.  
		 */
	...
	폼값받기2]  @RequestParam 어노테이션으로 폼값받기
		파라미터 형식으로 아래와 같이 사용한다. 어노테이션으로 폼값을 받은후
		우측의 변수에 즉시 대입한다. 따라서 메서드내에서 즉시 사용할 수 있다.
	 */
	...
	public String joinMember(Model model, 
		...
		//getParameter()로 받는 대신 어노테이션으로 받은후 변수에 저장한다.
		...
		//views폴더 하위의 01Form 디렉토리에 requestParam.jsp를 호출한다.
	...
	조건2 : 커맨드객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼
		형태의 매개변수를 사용해야 한다.
	...
	파라미터를 마치 경로명처럼 사용하게되고, 이때 주의해야 할 점은
